...
const CONFIG = {
 WEBSOCKET_ENDPOINT: process.env.WEBSOCKET_ENDPOINT || "wss://he7ifebjve.execute-api.us-east-1.amazonaws.com/production/",
 MAX_RETRIES: parseInt(process.env.MAX_RETRIES || '3'),
 RETRY_DELAY: parseInt(process.env.RETRY_DELAY || '1000'),
 CONNECTIONS_TABLE: process.env.CONNECTIONS_TABLE || "demoWebsocketConnections",
...
const problems = [
 ...
 },
 {
   ...
   solutionText: "Implement responsive design to improve the mobile experience.",
...
async function findConnectionIdForTask(taskId) {
 for (let attempt = 0; attempt < CONFIG.MAX_RETRIES; attempt++) {
   try {
     ...
     console.error(`Error finding connectionId for taskId (attempt ${attempt + 1}):`, error);
     if (attempt < CONFIG.MAX_RETRIES - 1) {
       await delay(attempt);
     }
   }
...
export const sendMessageToClient = async (connectionId, message, endpoint) => {
 if (!connectionId || !endpoint) {
   console.log('Skipping WebSocket message - missing connectionId or endpoint');
   return;
 }
...
export const updateTaskStatus = async (taskId, status, data = {}) => {
 ...
 const expressionAttributeValues = { ":status": status, ":timestamp": timestamp };
...
export const sendProgressUpdate = async (connectionId, endpoint, taskId, currentStep, progress) => {
...
export const handler = async (event) => {
 ...
 try {
   console.log("Lambda invocation with event:", JSON.stringify(event, null, 2));
   ...
   // Set initial status
   const statusUpdated = await updateTaskStatus(taskId, "processing");
   if (!statusUpdated) {
   ...
   async function sendProgressWithDelay() {
     for (const { step, progress, delay } of progressSteps) {
       console.log(`Sending progress update for ${step} with progress ${progress}`);
       if (connectionId) {
         await sendProgressUpdate(connectionId, parsedEvent.endpoint, taskId, step, progress);
         console.log(`Sent progress update for ${step} with progress ${progress}`);
       }
   ...
   // Analyze website and prepare results
   ...
   const finalUpdateSuccess = await updateTaskStatus(taskId, "completed", { problems: analysisResults });
